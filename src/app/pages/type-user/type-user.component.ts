...
export class TypeUserComponent implements OnInit {
  ...
  constructor(private nbMenuService: NbMenuService, private service: APIService, private router: Router) {
  ...
  aktifFront() {
    Swal.fire({
      title: 'Lock Item',
      text: "Apakah yakin Mengunci Type User Ini ?",
      type: 'question',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      ...
      if (result.value) {
        ...
        this.service.lockTypeUser(this.checkedItem).then(
          result => {
            console.log(result);
            Swal.fire(
              result.msg,
  ...
  nonAktifFront() {
    Swal.fire({
      title: 'Unlock Item',
      text: "Apakah yakin akan membuka kunci Type User Ini ?",
      type: 'question',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Yes, Unlock TypeUser!'
    }).then((result) => {
      if (result.value) {
        this.loading = true;
        this.loadingAction = true;
        this.service.unlockTypeUser(this.checkedItem).then(
          result => {
            console.log(result);
            Swal.fire(
              result.msg,
